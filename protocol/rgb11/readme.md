...
RGB11 uses an independent DKVS path so that its generation does not collide with account management or other modules. The core objects are:
...
- head and snapshot are written atomically to the target node through one `dkvsManager` batch-CAS;
...
If two devices modify state from the same old head, the later submission receives a head conflict, DKVS write conflict, or stale-generation error. The SDK does not merge two RGB11 states automatically. The application must select the latest valid state and re-execute the uncommitted operation.
...
- DKVS record identity, signature, sequence, PathGeneration, and fee proof;
...
Applications should call these domain APIs instead of constructing DKVS keys, records, sequences, PathGeneration values, or RGB11 internal storage objects directly.
